...
<body>
    <div class="header">
        ...
        <div>
            ...
            <button class="refresh-btn" onclick="show8PageSettings()" style="background-color: #9b59b6; margin-right: 10px;">📄 8-Page Redirect</button>
    ...
    <div class="container" id="8pageSettings" style="display: none;">
        ...
        <p style="color: #666; margin-bottom: 20px;">Configure short URLs to redirect through 8 randomized blog pages before reaching the final destination. This feature increases user engagement and blog visibility.</p>
        ...
        <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <div style="margin-top: 15px;">
                <button onclick="save8PageGlobalSettings()" style="background-color: #9b59b6; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">💾 Save Global Settings</button>
                <button onclick="load8PageSettings()" style="background-color: #17a2b8; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin-left: 10px;">🔄 Refresh</button>
            </div>
        </div>

        <div style="background: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            <h3 style="margin-top: 0; color: #856404;">📋 Page Configuration</h3>
            <p style="color: #856404; margin-bottom: 15px;">Configure each of the 8 pages in the redirection chain. Choose between internal blog posts or external blog URLs.</p>
            
            <div id="8pageConfigContainer">
                <!-- Dynamic page configuration will be loaded here -->
            </div>
            
            <div style="margin-top: 15px; text-align: center;">
                <button onclick="save8PageConfiguration()" style="background-color: #28a745; color: white; border: none; padding: 12px 24px; border-radius: 5px; cursor: pointer; font-size: 16px;">💾 Save All Page Settings</button>
            </div>
        </div>

        <div style="background: #e7f3ff; border: 1px solid #b6d7ff; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
            <h3 style="margin-top: 0; color: #0066cc;">📊 Analytics</h3>
            <div id="8pageAnalytics">
                <p>Loading analytics...</p>
            </div>
            <button onclick="load8PageAnalytics()" style="background-color: #007bff; color: white; border: none; padding: 8px 16px; border-radius: 4px; cursor: pointer;">📈 Refresh Analytics</button>
        </div>
    ...
    <script>
        ...
        async function load8PageSettings() {
            try {
                ...
                if (response.ok) {
                    const config = await response.json();
                    document.getElementById('8pageEnabled').value = config.enabled ? 'true' : 'false';
                    document.getElementById('8pageRandomize').value = config.randomize ? 'true' : 'false';
                    
                    await loadPageConfiguration(config.pages);
                    load8PageAnalytics();
                } else {
                    console.error('Failed to load 8-page settings');
                }
            } catch (error) {
        ...
        async function save8PageGlobalSettings() {
            try {
                ...
                if (response.ok) {
                    alert('Global 8-Page settings saved successfully!');
                    load8PageSettings(); // Refresh
                } else {
                    alert('Error saving settings');
                }
            } catch (error) {
        ...
        async function loadPageConfiguration(pages) {
            try {
                const token = localStorage.getItem('adminToken');
                
                // Load available blog posts
                const blogsResponse = await fetch('/admin/api/8page/available-blogs', {
                    headers: { 'Authorization': 'Bearer ' + token }
                });
                
                const availableBlogs = blogsResponse.ok ? await blogsResponse.json() : [];
                
                const container = document.getElementById('8pageConfigContainer');
                container.innerHTML = '';
                
                pages.forEach((page, index) => {
                    const pageDiv = document.createElement('div');
                    pageDiv.style.cssText = 'background: white; padding: 15px; margin: 10px 0; border-radius: 8px; border: 1px solid #ddd;';
                    
                    const blogOptions = availableBlogs.map(blog => 
                        '<option value="' + blog.id + '" ' + (page.id === blog.id ? 'selected' : '') + '>' + blog.title + ' (by ' + blog.author + ')</option>'
                    ).join('');
                    
                    pageDiv.innerHTML = 
                        '<h4 style="margin-top: 0; color: #495057;">📄 Page ' + (index + 1) + '</h4>' +
                        '<div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 15px; align-items: end;">' +
                            '<div>' +
                                '<label style="display: block; margin-bottom: 5px; font-weight: bold;">Status:</label>' +
                                '<select id="page' + index + 'Enabled" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">' +
                                    '<option value="true" ' + (page.enabled ? 'selected' : '') + '>Enabled</option>' +
                                    '<option value="false" ' + (!page.enabled ? 'selected' : '') + '>Disabled</option>' +
                                '</select>' +
                            '</div>' +
                            '<div>' +
                                '<label style="display: block; margin-bottom: 5px; font-weight: bold;">Type:</label>' +
                                '<select id="page' + index + 'Type" onchange="togglePageConfig(' + index + ')" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">' +
                                    '<option value="internal" ' + (page.type === 'internal' ? 'selected' : '') + '>Internal Blog</option>' +
                                    '<option value="external" ' + (page.type === 'external' ? 'selected' : '') + '>External URL</option>' +
                                '</select>' +
                            '</div>' +
                            '<div>' +
                                '<label style="display: block; margin-bottom: 5px; font-weight: bold;">Wait Time (sec):</label>' +
                                '<input type="number" id="page' + index + 'WaitTime" min="1" max="10" value="' + page.waitTime + '" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px;">' +
                            '</div>' +
                            '<div>' +
                                '<label style="display: block; margin-bottom: 5px; font-weight: bold;">Content:</label>' +
                                '<div id="page' + index + 'Content">' +
                                    '<select id="page' + index + 'BlogId" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; display: ' + (page.type === 'internal' ? 'block' : 'none') + ';">' +
                                        '<option value="">Select Blog Post</option>' +
                                        blogOptions +
                                    '</select>' +
                                    '<input type="url" id="page' + index + 'Url" placeholder="https://example.com/blog-post" value="' + (page.url || '') + '" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; display: ' + (page.type === 'external' ? 'block' : 'none') + ';">' +
                                '</div>' +
                            '</div>' +
                        '</div>';
        ...
        function togglePageConfig(pageIndex) {
            const type = document.getElementById(`page${pageIndex}Type`).value;
            const blogSelect = document.getElementById(`page${pageIndex}BlogId`);
            const urlInput = document.getElementById(`page${pageIndex}Url`);
        ...
        async function save8PageConfiguration() {
            try {
                const token = localStorage.getItem('adminToken');
                const pages = [];
                
                for (let i = 0; i < 8; i++) {
                    const enabled = document.getElementById(`page${i}Enabled`).value === 'true';
                    const type = document.getElementById(`page${i}Type`).value;
                    const waitTime = parseInt(document.getElementById(`page${i}WaitTime`).value) || 3;
                    
                    let id = null;
                    let url = null;
                    
                    if (type === 'internal') {
                        id = document.getElementById(`page${i}BlogId`).value || null;
                    } else {
                        url = document.getElementById(`page${i}Url`).value || null;
                    }
                    
                    pages.push({ enabled, type, id, url, waitTime });
                }
                ...
                if (response.ok) {
                    alert('8-Page configuration saved successfully!');
                    load8PageSettings(); // Refresh
                } else {
        ...
        async function load8PageAnalytics() {
            try {
                const token = localStorage.getItem('adminToken');
                const response = await fetch('/admin/api/8page/analytics', {
                    headers: { 'Authorization': 'Bearer ' + token }
                });
                
                if (response.ok) {
                    const analytics = await response.json();
                    const container = document.getElementById('8pageAnalytics');
                    
                    container.innerHTML = 
                        '<div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 15px; text-align: center;">' +
                            '<div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">' +
                                '<h4 style="margin: 0; color: #007bff;">Total Redirects</h4>' +
                                '<p style="font-size: 24px; font-weight: bold; margin: 5px 0; color: #333;">' + analytics.totalRedirects + '</p>' +
                            '</div>' +
                            '<div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">' +
                                '<h4 style="margin: 0; color: #28a745;">Completed Chains</h4>' +
                                '<p style="font-size: 24px; font-weight: bold; margin: 5px 0; color: #333;">' + analytics.completedChains + '</p>' +
                            '</div>' +
                            '<div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">' +
                                '<h4 style="margin: 0; color: #ffc107;">Completion Rate</h4>' +
                                '<p style="font-size: 24px; font-weight: bold; margin: 5px 0; color: #333;">' + analytics.completionRate + '%</p>' +
                            '</div>' +
                            '<div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">' +
                                '<h4 style="margin: 0; color: #dc3545;">Abandonment Rate</h4>' +
                                '<p style="font-size: 24px; font-weight: bold; margin: 5px 0; color: #333;">' + analytics.abandonnmentRate + '%</p>' +
                            '</div>' +
                        '</div>' +
                        (Object.keys(analytics.abandonedAt).length > 0 ? 
                            '<div style="margin-top: 15px;">' +
                                '<h4>Abandonment by Page:</h4>' +
                                Object.entries(analytics.abandonedAt).map(function(entry) {
                                    return '<span style="background: #fff3cd; padding: 3px 8px; margin: 2px; border-radius: 4px; border: 1px solid #ffeaa7; font-size: 12px;">' + entry[0] + ': ' + entry[1] + '</span>';
                                }).join('') +
                            '</div>'
                        : '');
                } else {
                    document.getElementById('8pageAnalytics').innerHTML = '<p style="color: #dc3545;">Error loading analytics</p>';
                }
            } catch (error) {
...
app.post('/admin/api/8page/config', requireAuth, (req, res) => {
  const { enabled, randomize, pages } = req.body;
  const ip = getClientIP(req);
  ...
  if (Array.isArray(pages) && pages.length === 8) {
    eightPageRedirectionConfig.pages = pages.map((page, index) => ({
      type: page.type === 'external' ? 'external' : 'internal',
      id: page.id || null,
      url: page.url || null,
      waitTime: Math.max(1, Math.min(10, parseInt(page.waitTime) || 3)),
      enabled: Boolean(page.enabled)
    }));
  }
  
  logAdminOperation('8PAGE_CONFIG_UPDATED', ip, { enabled, randomize, pagesUpdated: pages?.length || 0 });
...
// Get available blog posts for selection
app.get('/admin/api/8page/available-blogs', requireAuth, (req, res) => {
  const publishedPosts = Object.values(blogDatabase)
...
// Analytics endpoint for 8-page redirection
app.post('/admin/api/8page/analytics', (req, res) => {
  const { shortCode, pageIndex, abandoned } = req.body;
  
  if (abandoned) {
    // Track abandonment at specific page
    const page = `page_${pageIndex}`;
    eightPageRedirectionConfig.analytics.abandonedAt[page] = 
      (eightPageRedirectionConfig.analytics.abandonedAt[page] || 0) + 1;
  } else if (pageIndex === 0) {
    // Track start of new redirection chain
...
// Get 8-page redirection analytics
app.get('/admin/api/8page/analytics', requireAuth, (req, res) => {
  const analytics = {
    ...eightPageRedirectionConfig.analytics,
    abandonnmentRate: eightPageRedirectionConfig.analytics.totalRedirects > 0 ? 
      (Object.values(eightPageRedirectionConfig.analytics.abandonedAt).reduce((a, b) => a + b, 0) / 
       eightPageRedirectionConfig.analytics.totalRedirects * 100).toFixed(2) : 0,
    completionRate: eightPageRedirectionConfig.analytics.totalRedirects > 0 ?
...
app.get('/8page/:pageIndex/:shortCode', (req, res) => {
  ...
  if (!eightPageRedirectionConfig.enabled) {
    // If 8-page redirection is disabled, redirect directly
    recordClick(shortCode, req);
    return res.redirect(originalUrl);
  }
  ...
  if (currentPageIndex >= enabledPages.length) {
    recordClick(shortCode, req);
    eightPageRedirectionConfig.analytics.completedChains++;
    return res.redirect(originalUrl);
  }
  ...
  let currentPage;
  if (eightPageRedirectionConfig.randomize) {
    // Randomize page order but ensure we progress through all pages
    const seed = shortCode.charCodeAt(0) + currentPageIndex;
    const randomIndex = seed % enabledPages.length;
    currentPage = enabledPages[randomIndex];
  } else {
    currentPage = enabledPages[currentPageIndex];
  }
  
  // Generate the intermediate page
  res.send(generate8PageRedirectionPage(currentPageIndex, currentPage, shortCode, originalUrl, enabledPages.length));
});

// 8-Page redirection proceed route
app.get('/8page/proceed/:pageIndex/:shortCode', (req, res) => {
  const { pageIndex, shortCode } = req.params;
  const nextPageIndex = parseInt(pageIndex) + 1;
  
  // Redirect to next page in chain
  res.redirect(`/8page/${nextPageIndex}/${shortCode}`);
});

// Function to generate 8-Page redirection page HTML
function generate8PageRedirectionPage(pageIndex, pageConfig, shortCode, finalUrl, totalPages) {
  const nextPageIndex = pageIndex + 1;
  const progress = Math.round(((pageIndex + 1) / totalPages) * 100);
  
  let pageContent = '';
  
  if (pageConfig.type === 'internal' && pageConfig.id) {
    // Load internal blog post content
    const blogPost = blogDatabase[pageConfig.id];
    if (blogPost && blogPost.published) {
      const title = (blogPost.title || '').replace(/`/g, '\\`').replace(/\$/g, '\\$');
      const author = (blogPost.author || '').replace(/`/g, '\\`').replace(/\$/g, '\\$');
      const content = (blogPost.content || '').substring(0, 500).replace(/`/g, '\\`').replace(/\$/g, '\\$');
      const slug = blogPost.slug || '';
      pageContent = 
        '<div class="blog-content">' +
          '<h2>' + title + '</h2>' +
          '<div class="blog-meta">By ' + author + ' • ' + new Date(blogPost.createdAt).toLocaleDateString() + '</div>' +
          '<div class="blog-body">' + content + (blogPost.content.length > 500 ? '...' : '') + '</div>' +
          '<a href="/blog/' + slug + '" target="_blank" class="read-more">Read Full Article</a>' +
        '</div>';
    } else {
      pageContent = '<div class="blog-content"><h2>Blog Post Loading...</h2><p>Please wait while we prepare the content.</p></div>';
    }
  } else if (pageConfig.type === 'external' && pageConfig.url) {
    // Show external blog preview
    const url = (pageConfig.url || '').replace(/`/g, '\\`').replace(/\$/g, '\\$');
    pageContent = 
      '<div class="blog-content">' +
        '<h2>Recommended Reading</h2>' +
        '<p>We recommend checking out this interesting article:</p>' +
        '<div class="external-link">' +
          '<a href="' + url + '" target="_blank" rel="noopener noreferrer">' + url + '</a>' +
        '</div>' +
        '<p>This will open in a new tab so you can continue your journey here.</p>' +
      '</div>';
  } else {
    pageContent = 
      '<div class="blog-content">' +
        '<h2>Preparing Content...</h2>' +
        '<p>Please wait while we load the next piece of content for you.</p>' +
      '</div>';
  }
...
'<head>' +
    '<meta charset="UTF-8">' +
    '<meta name="viewport" content="width=device-width, initial-scale=1.0">' +
    '<title>Content Hub - Page ' + (pageIndex + 1) + ' of ' + totalPages + '</title>' +
    '<style>' +
        'body {' +
            'font-family: Arial, sans-serif;' +
            'margin: 0;' +
...
'<body>' +
    '<div class="container">' +
        ...
        '<div class="page-counter">Page ' + (pageIndex + 1) + ' of ' + totalPages + '</div>' +
        '<div class="progress-bar">' +
            '<div class="progress-fill"></div>' +
        '</div>' +
        pageContent +
        '<div class="countdown-section">' +
            '<div>Please wait <span id="countdown">' + pageConfig.waitTime + '</span> seconds</div>' +
            '<div style="font-size: 14px; margin-top: 10px;">Loading next content...</div>' +
        '</div>' +
        '<a href="/8page/proceed/' + pageIndex + '/' + shortCode + '" class="proceed-btn" id="proceedBtn">' +
            (nextPageIndex >= totalPages ? '🎯 Continue to Destination' : '➡️ Next Content') +
        '</a>' +
        '<div class="destination-info">' +
            '<strong>Final destination:</strong> ' + finalUrl +
        '</div>' +
    '</div>' +
    '<script>' +
        'let timeLeft = ' + pageConfig.waitTime + ';' +
        ...
        '}, 1000);' +
        'if (' + pageIndex + ' > 0) {' +
            'fetch("/admin/api/8page/analytics", {' +
                'method: "POST",' +
                'headers: { "Content-Type": "application/json" },' +
                'body: JSON.stringify({ ' +
                    'shortCode: "' + shortCode + '",' +
  ...
  if (originalUrl) {
    ...
    if (eightPageRedirectionConfig.enabled) {
      ...
      if (enabledPages.length > 0) {
        // Start the 8-page redirection chain
